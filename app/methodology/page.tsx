...
export default function MethodologyPage() {
    ...
    const texts = {
        en: {
            ...
            modelClassificationDesc: 'We classify models into three categories based on their open-source status and evaluation methodology:',
            standardOpensource: 'Standard Open-Source Models',
            standardOpensourceDesc: 'Models that follow the standard evaluation protocol AND have publicly available code. These models are displayed by default in the leaderboards as they provide the highest level of reproducibility and transparency.',
            standardClosed: 'Standard Closed-Source Models',
            standardClosedDesc: 'Models that follow the standard evaluation protocol but do NOT have publicly available code. These models are hidden by default but can be shown using the "Include Closed-Source Models" toggle. While their results follow standard protocols, they cannot be independently verified or reproduced.',
            nonStandard: 'Non-Standard Evaluation Models',
            nonStandardDesc: 'Models that did NOT follow the standard evaluation protocol (e.g., different task subsets, modified evaluation settings, or missing metrics). These models are shown in the "Appendix" section and their results may not be directly comparable with other models.',
        ...
        zh: {
            ...
            modelClassificationDesc: '我们根据模型的开源状态和评估方法将模型分为三类：',
            standardOpensource: '标准开源模型',
            standardOpensourceDesc: '遵循标准评估协议且代码公开可用的模型。这些模型在排行榜中默认显示，因为它们提供了最高水平的可复现性和透明度。',
            standardClosed: '标准闭源模型',
            standardClosedDesc: '遵循标准评估协议但代码未公开的模型。这些模型默认隐藏，但可以通过"包含闭源模型"开关显示。虽然其结果遵循标准协议，但无法独立验证或复现。',
            nonStandard: '非标准测试模型',
            nonStandardDesc: '未遵循标准评估协议的模型（例如：不同的任务子集、修改的评估设置或缺失的指标）。这些模型显示在"附录"部分，其结果可能无法与其他模型直接比较。',
            starUs: '支持本项目',
